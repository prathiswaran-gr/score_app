...
class Database {
  ...
  Future<void> getTodayAllMatches() async {
    ...
    List<dynamic> list = [];
    var allMatches = await matches
        .doc("matchDocumentId")
        .collection(getDateFormat())
        .doc(getDateFormat() + "Id")
        .get();
  ...
  Future<void> addMatchDetails() async {
    MatchDetails matchDetails1 = MatchDetails(
        matchName: "abc",
        date: getDateFormat(),
        time: "8:00 AM",
        opponent1: "India",
  ...
  Future<Map<String, dynamic>> getUpcommingMatches() async {
    List<dynamic> list = [];
    CollectionReference liveMatches =
        FirebaseFirestore.instance.collection("LiveMatches");
    var snapshot = liveMatches.doc("live").get();
    await snapshot.then((value) => list.add(value.data()));
    print(list);
    return list[0];
  }
}
